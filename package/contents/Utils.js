...
function errorRaised(err, message, icon) {
  ...
  error = i18n(err);
  statusIcon = icon ? icon : "rdockr-error";
}

//Util functions
function notificationInstall() {
  let notifypath = "~/.local/share/knotifications6/rdockr.notifyrc";
  let notifycontent = `
[Global]
...
Sound=message-new-instant.ogg`;
  dockerCommand.executable.exec(
    "mkdir -p ~/.local/share/knotifications6/",
    (_, _, _, _) => { },
  );
  dockerCommand.executable.exec(
    "echo '" + notifycontent + "' > " + notifypath,
    (_, _, _, _) => { },
  );
}
...
function invokeDelayTimerCallback(callback) {
  if (typeof delayTimerCallback === "function") {
    ...
    if (cfg.debug) {
      console.log("delayTimerCallback is not defined or not a function");
    }
  }
}
  ...
  exec(cmd, ...args) {
    dockerCommand.executable.exec(cmd, (_, stdout, stderr, _2) => {
      ...
      if (cfg.debug) {
        ...
        console.log(
          "Response Code:",
          resCode || "No match found for response code",
        );
      }
...
let commands = {
  statDocker: new Command(
    "curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X GET http://localhost/containers/json",
    "stat-docker",
    statDockerCallback,
  ),
  startDocker: new Command(
    "systemctl start docker.service docker.socket && curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X GET http://localhost/containers/json",
    "start-docker",
    initDocker,
  ),
  stopDocker: new Command(
    "systemctl stop docker.service docker.socket && curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X GET http://localhost/containers/json",
    "stop-docker",
    initDocker,
  ),
  startContainer: new Command(
    "curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X POST http://localhost/containers/{}/start",
    "start-container",
    startContainerCallback,
  ),
  stopContainer: new Command(
    "curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X POST http://localhost/containers/{}/stop",
    "stop-container",
    stopContainerCallback,
  ),
  restartContainer: new Command(
    "curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X POST http://localhost/containers/{}/restart",
    "restart-container",
    restartContainerCallback,
  ),
  deleteContainer: new Command(
    "curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X DELETE http://localhost/containers/{}",
    "delete-container",
    deleteContainerCallback,
  ),
  inspectContainer: new Command(
    "curl -s --unix-socket /var/run/docker.sock --write-out 'Response:%{http_code}' -X GET http://localhost/containers/{}/json",
    "inspect-container",
    inspectContainerCallback,
  ),
};
...
function statDockerCallback(resCode) {
  if (resCode === "200") {
    dockerEnable = true;
    if (cfg.debug) {
      console.log("Docker Enable: " + dockerEnable + " | Docker is running");
    }

    statusMessage = "Docker is running";
    statusIcon = "rdockr-rocket";

    updateDockerAction("Stop Docker Engine", "rdockr-stop", "stopDocker");
  } else if (resCode !== "200" || resCode === "") {
    endAll();
    dockerEnable = false;
    if (cfg.debug) {
      console.log(
        "Docker Enable: " + dockerEnable + " | Docker is not running",
      );
    }

    statusMessage = "Docker is not running";
    statusIcon = "rdockr-error";

    updateDockerAction("Start Docker Engine", "rdockr-start", "startDocker");
...
function startContainerCallback(resCode, containerId, containerName) {
  ...
  if (resCode === "204") {
    ...
    errorMessage =
      `Failed to start container name: ${containerName} id: ${containerId}. Response: ${resCode}`;
  }

  if (cfg.debug) {
    console.log("start container callback error message: " + errorMessage);
  }
...
function stopContainerCallback(resCode, containerId, containerName) {
  ...
  if (resCode === "204") {
    ...
    errorMessage =
      `Failed to stop container name: ${containerName} id: ${containerId}. Response: ${resCode}`;
  }

  if (cfg.debug) {
    console.log("stop container callback error message: " + errorMessage);
  }
  errorRaised(errorMessage);
}
...
function restartContainerCallback(resCode, containerId, containerName) {
  ...
  if (resCode === "204") {
    ...
    errorMessage =
      `Failed to restart container name: ${containerName} id: ${containerId}. Response: ${resCode}`;
  }

  if (cfg.debug) {
    console.log("restart container callback error message: " + errorMessage);
  }
  errorRaised(errorMessage);
}
...
function deleteContainerCallback(resCode, containerId, containerName) {
  ...
  if (resCode === "204") {
    ...
    errorMessage =
      `You cannot remove a running container: ${containerName}. Stop the\ncontainer before attempting removal or force remove.`;
  } else if (resCode === "500") {
    errorMessage = "Internal server error.";
  } else {
    errorMessage =
      `Failed to delete container name: ${containerName} id: ${containerId}. Response: ${resCode}`;
  }

  if (cfg.debug) {
    console.log("delete container callback error message: " + errorMessage);
  }
  errorRaised(errorMessage);
}
...
function inspectContainerCallback(resCode, containerId, containerName, stdout) {
  ...
  if (cfg.debug) {
    console.log("inspect container callback error message: " + errorMessage);
  }
  errorRaised(errorMessage);
}
